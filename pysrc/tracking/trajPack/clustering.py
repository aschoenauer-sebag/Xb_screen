...
#                        time_length.extend([len(coord[k][0]) for k in filter(lambda x: x not in toDel, range(len(coord)))])
                        ...
                        coordinates.append(len(toDel)/float(len(ll)))
...
'''
    ...
    if options.action=='collectingTrajectories':
        ...
        print "CAREFUL HERE WE TAKE THE SIX FIRST PRINCIPAL COMPONENTS BY DEFAULT"
        ...
        print 'BatchKMeans'
        silhouette_r, cohesion_r = BatchKmeans(narr[:,:6], 2, 30, N=10)
        f=open(os.path.join(output_folder, '{}3_batchKM_{}.pkl'.format(options.outputname, options.n_iter)), 'w')
