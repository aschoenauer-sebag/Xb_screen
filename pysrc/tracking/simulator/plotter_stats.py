...
class ScatterplotMatrix(object):
    """
    Makes a scatter plot matrix with histograms on the main diagonal and scatter plots 
    as off-diagonal elements.
    
    Example: 
    >>> import histopath.plotter.stats
    >>> sm = histopath.plotter.stats.ScatterplotMatrix()
    >>> X = np.concatenate([np.random.normal(1.0, 1.5, 30), np.random.normal(3.0, 1.0, 20)])
    >>> Y = np.concatenate([np.random.normal(-0.5, 0.8, 30), np.random.normal(4.0, 2.0, 20)])
    >>> Z = 2.0 * (X - Y) + np.random.normal(0.0, 1.0, 50)
    >>> datamatrix = np.vstack([X, Y, Z]).T
    >>> sm(datamatrix, filename='/Users/twalter/sm.png', colnames=['X', 'Y', 'Z'], color=['red' for i in range(30)] + ['blue' for i in range(20)])
    """
    def __call__(self,
                 datamatrix,
                 filename,
        ...
        # new figure
        fig = plt.figure(1, figsize=(scaling_factor*nb_cols, scaling_factor*nb_cols))
        ax = plt.subplot(1,1,1)
            ...
            for j in range(nb_cols):
                ...
                if i == j:
                    ...
                    ax.scatter(datamatrix[:,j], datamatrix[:,i], color=color,
                               alpha = histo_alpha, s=marker_size,
