...
from tracking.importPack import FEATURE_NUMBER, EVENTS, k, dmax, XMAX, YMAX
from tracking.PyPack import fHacktrack_reverse_vigra_saving
from tracking.dataPack.joining import FeatureException, MoreThanOneException

compteur = 0

def addFrom(size, plate, well, index, l, lstCellsF):
    if plate not in lstCellsF[size]:
        lstCellsF[size][plate]={}
    if well not in lstCellsF[size][plate]:
        lstCellsF[size][plate][well]={}
    if index not in lstCellsF[size][plate][well]:
        lstCellsF[size][plate][well][index]=[]
    for i in l:
#        print i
        if int(i)!=-1:
            lstCellsF[size][plate][well][index].append(i)

    
def dist(centers):
    moy =[]#; count = 0
    for center1, center2 in combinations(centers, 2):
        moy.append(sqrt((center1[0]-center2[0])**2 + (center1[1]-center2[1])**2))
        #count+=1
    return moy

def moyMultipleCenters(centers):
    x=[]
    y=[]
    for c in centers:
        x.append(c[0])
...
def importSegOnly(filename, plaque, puits):
    pathSeg = "/sample/0/plate/"+plaque+"/experiment/"+puits[:-3]+"/position/"+puits[-1]+"/image/region"
...
#   
    ...
    if int(tabFeatures.shape[0])!=FEATURE_NUMBER and featureNumberTotal:
        raise FeatureException('There is a problem with the size of features array')
        
    return np.array([el[0] for el in tabFeatures])        
...
#    p.scatter(range(len(dmoySplit)), dmoySplit[0], marker='^', color='red', hold='on')
#    p.scatter(range(len(dmoyMerge)), dmoyMerge[0], marker='o')
#    p.scatter(range(len(dmoySplit)), dmoySplit[1], marker='^', color='black', hold='on')
#    p.scatter(range(len(dmoyMerge)), dmoyMerge[1], marker='o')
#    p.scatter(range(len(dmoySplit)), dmoySplit[2], marker='^', color='green', hold='on')
#    p.scatter(range(len(dmoyMerge)), dmoyMerge[2], marker='o')
#    p.xlabel('Number of occurences')
#    p.ylabel("Distance on the same frame, merges and splits")
#    p.title('Training set size :'+str(compteur)+" objects")
#    p.grid(True)
#    p.show()
    
#        2.75928556131 26.2366982119 23.3638524226 5.85474592903 5.92799796592
#        3.47423344097 6.75227021644 6.96428812355 4.29774040383 5.06596245986

#        25316 489 509
#2.75928556131 5.85474592903 5.92799796592
#3.47423344097 4.29774040383 5.06596245986
#moyenne pour les splits de taille 0   26.3835192189
#std pour les splits de taille 0   6.40480301098
#moyenne pour les merges de taille 0   22.8720354913
#std pour les merges de taille 0   6.21313474285
#moyenne pour les splits de taille 1   25.5823362289
#std pour les splits de taille 1   8.87414346739
#moyenne pour les merges de taille 1   24.2430019932
#std pour les merges de taille 1   6.91317625064
#moyenne pour les splits de taille 2   23.9705495171
#std pour les splits de taille 2   6.38979049072
#moyenne pour les merges de taille 2   37.8733290806
#std pour les merges de taille 2   14.7459919193
#25316 489 509

#le 26/02
#2.98389937683 5.87423301819 5.79652616944
#3.54502112138 4.16286913833 4.86843508678
#moyenne pour les splits de taille 0   26.1706636784
#std pour les splits de taille 0   6.54727201185
#moyenne pour les merges de taille 0   22.6442195154
#std pour les merges de taille 0   5.94758343938
#moyenne pour les splits de taille 1   25.6235167699
#std pour les splits de taille 1   8.44435807006
#moyenne pour les merges de taille 1   23.4623778707
#std pour les merges de taille 1   7.20401117652
#/usr/local/lib/python2.7/dist-packages/numpy/core/fromnumeric.py:37: RuntimeWarning: invalid value encountered in double_scalars
#  result = getattr(asarray(obj),method)(*args, **kwds)
#moyenne pour les splits de taille 2   nan
#std pour les splits de taille 2   nan
#moyenne pour les merges de taille 2   nan
#std pour les merges de taille 2   nan
#29654 614 652
